...
class EKF(object):
    ...
    def step1__calculate_a_priori(self,F,Q,G,U,isinitial=False):
        ...
        if not isinitial:
            FT = F.T
            xhatminus = dot(F,self.xhat_k1)+dot(G,U)
            # compute a priori estimate of errors
            Pminus = dot(dot(F,self.P_k1),FT)+Q
        else:
...
##             print
    ...
    def step(self,F,Q,G,U,y,hx,C,R):
        xhatminus, Pminus = self.step1__calculate_a_priori(F,Q,G,U,isinitial=False)
        return self.step2__calculate_a_posteriori(xhatminus, Pminus, y, hx, C, R)

EKFl=EKF(initial_x=Xstate, initial_P=Pmatrix)
